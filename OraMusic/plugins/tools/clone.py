...
from OraMusic.utils.database import get_assistant, clonebotdb
from OraMusic.utils.database.clonedb import has_user_cloned_any_bot, get_owner_id_from_db
...
def get_random_start_img():
    ...
    return "https://files.catbox.moe/6r97s4.jpg" # Fallback
...
async def delayed_start(bot_token, session_string, wait_time, bot_number):
    ...
    try:
        ai = Client(
            ...
            api_hash=API_HASH,
            bot_token=bot_token,
            plugins=dict(root="OraMusic.cplugin"),
            in_memory=True,
        )
        await ai.start()
